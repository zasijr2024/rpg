...
Current verdict: **HOLD for public Release Candidate sign-off**. Revision `d3696de28218bb6c7645302398e1a4b5fe7cba18`, its clean technical-RC pass, and its policy corpus are historical evidence; remediation after that checkpoint superseded it as a release candidate. P14V-02's Phase 0 audit classified every dirty path, and the maintainer approved the three exact groups, selected candidate package version `0.1.0-rc.1`, and authorized only their listed Git writes. A new post-remediation revision/artifact must now be reproduced before receiving a separately named 32-seed corpus. Hosted CI has not yet run, no schema-v3 unassisted sessions are recorded, and Space has not been flown end-to-end with a real screen reader on the new candidate. Repository-side MPL-2.0, NOTICE, public artifact copies, and source-derived inventory are implemented; the durable exact-source URL, any distribution-specific legal review, product/release-owner sign-off, production-host smoke, and final tag remain external/open.
...
| P14V-02 | Scope-safe checkpoint and clean reproduction       | P14V-01                            | maintainer             | Reviewed coherent commits exist on one post-remediation revision; a separate clean checkout with initialized submodules passes `npm ci` and the technical RC gate                                                             | superseded; new reproduction required           |
| P14V-03 | Hosted CI validation                               | P14V-02                            | maintainer/repo admin  | Change-lane and manual full technical-RC workflows pass on the same revision; the stable always-reporting required context is enforced; run/ruleset URLs/IDs are recorded                                                     | blocked - no new candidate or configured remote |
| P14V-04 | Progression-policy validity                        | P14V-02                            | engineering            | Every current four-seed result is classified; legal death and Workshop recovery are represented; remaining failures distinguish policy, game defect, and source-authentic friction                                            | done - method current; candidate reruns in RC   |
| P14V-05 | Fixed 32-seed progression corpus                   | P14V-02, P14V-04                   | engineering            | A separately named candidate-specific 32-seed summary records milestones, deaths, combats, events, bottlenecks, and classifications; no verified game-origin hard/soft lock remains                                           | reopened - replacement corpus required          |
| P14V-06 | Unassisted production playtests                    | P14V-03, P14V-05                   | playtest operators     | At least 5 valid schema-v3 first-time records are bound to one frozen revision/artifact/cohort/ruleset/mode policy; continue up to 8 only under the preregistered inconsistency rule                                          | new cohort/artifact required; 0/5               |
| P14V-07 | Real screen-reader Space and ending pass           | P14V-03, P14V-05                   | accessibility operator | All runbook scenarios pass on the candidate revision, including a complete real-time nonvisual Space flight and score ending; the evidence template has no `PENDING` fields                                                   | waiting for automated gates and operator        |
...
Current requirement: after P14V-02 freezes the replacement candidate, run the formal corpus without `--allow-dirty` and write a new date/SHA-qualified JSON plus interpretation record. Do not overwrite the historical pair. P14V-05 closes only when the replacement candidate's artifact validates and every stop is classified.
