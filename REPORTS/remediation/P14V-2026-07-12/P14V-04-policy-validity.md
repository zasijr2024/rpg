...
That won-combat transition was corrected. A subsequent full focused rerun did not return after the 600-second per-seed allowance and was terminated without a final JSON summary. It is not retained as outcome evidence and makes no claim that Engineering, kinetic armour, or Martial completed. At that historical iteration, it established that the end-to-end probe remained too opaque and too slow for P14V-05 despite the snapshot improvement; live checkpoint mode became the next required diagnostic before another full rerun.
