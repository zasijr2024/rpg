...
export function originalVillageTitleForHuts(huts: number): string {
  return originalVillageTitleThresholds.find(
    (threshold) => huts <= threshold.maxHuts,
  )?.title as string;
}
...
export function originalPopulationIncrease(
  space: number,
  roll: number,
): number {
  if (space <= 0) return 0;
  const arrivals = Math.floor(roll * (space / 2) + space / 2);
  return arrivals === 0 ? 1 : arrivals;
}

export function originalPopulationDelayMinutes(roll: number): number {
  return (
    Math.floor(roll * (OUTSIDE_POP_DELAY_MAX - OUTSIDE_POP_DELAY_MIN)) +
...
function assertKeysMatchManifest(): void {
  ...
  if (
    workerKeys.join("\u0000") !== canonicalManifest.keys.workers.join("\u0000")
  ) {
    throw new Error(
      "Original outside worker keys do not match canonical manifest",
    );
  }
}
