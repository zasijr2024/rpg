...
Operational sequence: `status/phase-14-post-remediation-next-steps-2026-07-30.md`.
This document owns executable gate semantics, not package scheduling or external
release authority.
...
The executable `Technical Release Candidate` gate is an automated code and reproducibility gate. The later Phase 14 roast identified evidence that cannot be honestly reduced to that command. Public Release Candidate sign-off additionally follows `status/phase-14-release-readiness-plan-2026-07-12.md`: hosted CI and the enforced required-check context must run on the candidate SHA, the progression policy must be validated before interpreting a fixed candidate-specific 32-seed corpus, at least five strict schema-v3 first-time unassisted sessions must share one frozen revision/artifact/cohort/ruleset/mode policy (continuing up to eight under a preregistered conflict rule), and a real screen reader must complete Space and the ending on the normal runtime clock. The product/release owner must verify the shipped MPL-2.0, NOTICE, and source-derived inventory; close the durable exact-source URL and any distribution-specific legal review; and record the final mode/public-distribution decision.

P14V-01 makes that boundary executable. `gate:rc` is explicitly technical, `study:progression` identifies itself as a policy diagnostic, the version-3 human gate validates one exact cohort plus reconciled wall/foreground/background/closed time and mode exposure, and the manual Space fixture runs the normal clock without a console API. The separate `study:progression:corpus` command owns P14V-05: it shards the fixed corpus, rejects incomplete or inconsistent coverage, emits one versioned aggregate, and refuses a dirty candidate unless a provisional operator explicitly supplies `--allow-dirty`. `closure:verify-tag` fails closed for open RA, P14R, and P14V packages. A green technical gate still must not be reported as human pacing, real assistive-technology, or public-release approval.
...
Candidate lineage distinguishes product candidate `C` and artifact identity `A`
from a later evidence-only descendant `F`. Evidence may carry to `F` only when
the diff is restricted to evidence/status/session records and a clean rebuild
still has identity `A`. Product, public asset, build/configuration, dependency,
workflow, runtime, balance, or artifact drift creates a new candidate and
...
The later roast-remediation integration passed 72 files / 508 tests, parity, types, lint, formatting, the tightened production build, 15/15 served-`dist` executions across Chromium/Firefox/WebKit, 1/1 complete production spine, 12/12 focused cross-browser accessibility scans, and both dependency audits with zero vulnerabilities. The original four-seed progression policy completed 0/4 and classified every stop; after policy/runtime corrections, clean candidate `d3696de` reproduced 4/4 with 11 legal deaths and no failures. Its separate retained 32-seed corpus records 12 study-policy completions, 20 policy-classified stops, and zero game-defect/unclassified stops. None is a player statistic.

Historical revisions `d3696de` and `6de3979` retain revision-specific clean gate and corpus evidence. Current candidate `275c096247e5fe2026e00c1f67eb78cd4668ccaf` passed the clean Node 24 technical gate, its separately named 32-seed P14V-05 corpus, both hosted workflow lanes, and enforced required-check validation. Public sign-off remains `HOLD` pending five schema-v3 human sessions, real-screen-reader evidence, exact-source/legal-owner and product decisions, production-host smoke, and final tag authorization/verification.

The 2026-07-30 clean candidate gate passed 77 files / 550 unit, content, and tooling tests after evidence-contract hardening; types, lint, formatting, and parity generation; a production build at 436,405 B / 123,783 B gzip initial JavaScript and 23,614 B CSS; 15/15 served-production checks; the complete production spine; 33/33 release-browser checks; 15/15 focused accessibility checks; 1/1 performance; and 410 desktop parity executions with 166 intentional skips. The four-seed policy diagnostic completed 4/4 with 11 legal deaths and no classified failures. Both dependency audits reported zero vulnerabilities. The candidate artifact is `sha256:147e06733788a771a8a3598c383b0f7b2103fec705ac6dabb10f101f3a95386c` across 16 files / 646,179 bytes.

P14V-02, P14V-03, and P14V-05 are complete on candidate `275c096` with unchanged artifact `sha256:147e06733788a771a8a3598c383b0f7b2103fec705ac6dabb10f101f3a95386c`. Pull-request run `30700296963`, manual full-RC run `30700299995`, and active strict ruleset `20083779` own the hosted proof. P14V-06 and P14V-07 are open for human and real assistive-technology operators; overall phase closure remains blocked by those and later owner/host/tag packages.

P14V-09 is a pre-tag authorization package rather than a self-referential claim
that a future tag already passed. Once its final manifest is `done`, create the
authorized local annotated tag, run `closure:verify-tag`, publish only after
`PASS`, and retain the resolved tag/SHA as post-tag evidence. The tagged commit
cannot contain its own future verification result.
