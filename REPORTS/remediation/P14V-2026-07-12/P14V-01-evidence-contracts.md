...
| Strict human cohort | Schema version 2 requires an exact 40-character revision, cohort/ruleset/artifact identity, consent, unassisted execution, environment, active-play sittings, milestones, structured deaths, bottlenecks, technical exceptions, and exclusions. The validator rejects unexpected fields, coached or excluded records, duplicate session/participant IDs, inconsistent completion data, and mixed cohorts. | `playtest-summary.test.ts` passes malformed-record, duplicate-participant, mixed-revision, and summary checks. `study:human` honestly reports zero collected sessions. |
